...
{
    public class AviationStackAirportsRepository : IAirportsRepository
    {
        protected readonly IConfiguration Configuration;

        public AviationStackAirportsRepository(IConfiguration configuration)
        {
            Configuration = configuration;
        }

        public Task<int> BulkInsert(List<Airport> flight)
        {
        ...
        {
            AviationEndPoint endpoint = Configuration.GetSection("AviationEndPoints").Get<List<AviationEndPoint>>().Find(Endpoint => Endpoint.Name == "aviationstack.com-airports");
            ...
            using (HttpClient client = new HttpClient(handler))
            {
                ...
                {
                    var apiResult = await response.Content.ReadFromJsonAsync<AviationstackAirportsApiResponse>();
